...
<head>
    ...
    {{-- Jewelry Shop Theme CSS --}}
    <link rel="stylesheet" href="{{ asset('templates/css/jewellery-shop-vcard.css') }}">
...
<body>
    ...
    @include('templates.includes.vcard.jewellery-shop.bg-icons')

    <div class="vcard-container" id="main-vcard">

        {{-- ── COVER ── --}}
        <div class="cover-photo">
            @if ($business_card_details->cover_type == 'none')
                <div class="cover-media-item"
                    style="background:url('{{ url('img/templates/jewellery-shop-vcard/banner.png') }}') center/cover no-repeat;">
                </div>
            @endif
        ...
        {{-- Language Switcher — outside cover so iframe never blocks it --}}
        @if (
            $business_card_details->is_enable_language_switcher == 1 &&
                is_array(config('app.languages')) &&
                count(config('app.languages')) > 1)
            @include('templates.includes.vcard.jewellery-shop.language-switcher')
        @endif
...
@endif
                            ...
                            @if (in_array($feature->type, ['tel', 'address', 'wa', 'instagram']))
                                @if ($feature->type == 'address')
                                    <a href="#location" class="action-btn">
                                        <span class="action-btn__icon"><i class="{{ $feature->icon }}"></i></span>
                ...
                {{-- ── FEATURE LINKS — Magazine card layout ── --}}
                @php
                ...
                @if ($feature_details->where('type', 'youtube')->count() > 0)
                    <div class="section gsap-fade">
                        <div class="section-header">
                            <h2 class="section-title">{{ __('Virtual Tours') }}</h2>
                            <div class="section-deco"><span class="section-deco__dash"></span><span
                                    class="section-deco__diamond"></span><span class="section-deco__dash"></span>
                            </div>
                        </div>
                                            ...
                                            <div class="item-card">
                                                @if ($feature->label)
                                                    <div class="youtube-label">
                                                        <p class="youtube-label-text">{{ $feature->label }}</p>
                                                    </div>
                                                @endif
                                                <iframe src="https://www.youtube.com/embed/{{ $feature->content }}"
                ...
                @if (count($service_details) > 0)
                    <div class="section gsap-fade">
                        <i class="far fa-gem decor-icon anim-float" style="font-size:85px;left:0;top:20px"></i>
                        <div class="section-header">
                            ...
                            <div class="swiper-wrapper">
                                @foreach ($service_details as $s)
                                    <div class="swiper-slide">
                                        <div class="item-card">
                                            <div
                                                style="width:100%;aspect-ratio:16/9;background:url('{{ url($s->service_image) }}') center/cover no-repeat;">
                                            </div>
                                            <div class="item-card__body">
                                                <h4>{{ $s->service_name }}</h4>
                                                <p style="margin-top:4px">{{ $s->service_description }}</p>
                                                @if ($enquiry_button != null && $whatsAppNumberExists == true && $s->enable_enquiry == 'Enabled')
                                                    <a href="https://wa.me/{{ $enquiry_button }}?text={{ __('Hi, I am interested in your product/service:') }} {{ $s->service_name }}. {{ __('Please provide more details.') }}"
                                                        target="_blank" class="btn-primary btn-sm"
                                                        style="margin-top:14px">{{ __('Enquire') }}</a>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="swiper-slide">
                                        <div class="item-card" style="position:relative">
                                            ...
                                            <div class="item-card__body">
                                                ...
                                                <p style="margin-top:4px">{{ $p->product_description }}</p>
                                                @if ($p->sales_price != 0)
                                                    <span
                                                ...
                                                @if ($p->product_status != 'null')
                                                    <p
                                                        style="font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:8px;color:{{ $p->product_status == 'instock' ? '#1A6840' : '#8A2A1A' }}">
                                                        {{ $p->product_status == 'outstock' ? __('Out of Stock') : __('In Stock') }}
                                                    </p>
                                                @endif
                                                @if ($enquiry_button != null && $whatsAppNumberExists == true)
                                                    <a href="https://wa.me/{{ $enquiry_button }}?text={{ __('Hi, I am interested in your product:') }} {{ $p->product_name }}. {{ __('Please provide more details.') }}"
                                                        target="_blank" class="btn-primary"
                                                        style="padding:10px;font-size:10px">{{ __('Enquire') }}</a>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                ...
                @if (
                    ...
                    <div class="section gsap-fade" id="booking-section">
                        <div class="section-header">
                            <h2 class="section-title">{{ __(json_decode($appointment_slots, true)['title']) }}</h2>
                            <div class="section-deco"><span class="section-deco__dash"></span><span
                                    class="section-deco__diamond"></span><span class="section-deco__dash"></span>
                            </div>
                        </div>
                        <div class="gem-card" style="padding:24px">
                ...
                @if ($plan_details['business_hours'] == 1 && $business_hours != null && $business_hours->is_display != 0)
                    <div class="section gsap-fade">
                        ...
                        <div class="gem-card" style="padding:20px 22px">
                            ...
                            @else
                                <div style="display:flex;align-items:center;gap:16px;padding:8px 0">
                                    <i class="far fa-gem" style="color:var(--rose);font-size:22px"></i>
                                    <div>
                                        <p
                                            style="font-family:'Bodoni Moda','Georgia',serif;font-style:italic;font-weight:700;font-size:16px;color:var(--text-dark)">
                                            {{ __('Always Open') }}</p>
                    ...
                    <div class="section gsap-fade">
                        ...
                        <div class="gem-card" style="padding:24px">
                            ...
                            <div class="service-booking-form">
                                <div><label class="form-label">{{ __('Name') }}</label><input type="text"
                                        name="customer_name" id="customer_name" placeholder="{{ __('Your Name') }}"
                                        class="form-control" /></div>
                                <div><label class="form-label">{{ __('Email') }}</label><input type="email"
                                        name="customer_email" id="customer_email"
                                        placeholder="{{ __('Your Email') }}" class="form-control" /></div>
                                <div><label class="form-label">{{ __('Mobile') }}</label><input type="tel"
                                        name="customer_phone" id="customer_phone" placeholder="{{ __('Mobile') }}"
                                        class="form-control" /></div>
                                <div><label class="form-label">{{ __('Persons') }}</label><input type="number"
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
                                <div><label class="form-label">{{ __('Start Date') }}</label><input type="text"
                                        id="service_start_date" name="service_start_date" class="form-control" />
                                </div>
                                <div><label class="form-label">{{ __('Start Time') }}</label><input type="time"
                                        name="service_start_time" id="service_start_time"
                                        class="form-control timepicker" /></div>
                                <div><label class="form-label">{{ __('End Date') }}</label><input type="date"
                                        id="service_end_date" name="service_end_date" class="form-control" /></div>
                                <div><label class="form-label">{{ __('End Time') }}</label><input type="time"
                                        name="service_end_time" id="service_end_time"
                                        class="form-control timepicker" /></div>
                            </div>
                                ...
                                @foreach ($testimonials as $t)
                                    <div class="swiper-slide">
                                        <div class="testimonial-card">
                                            <p
                                                style="font-family:'Bodoni Moda','Georgia',serif;font-style:italic;font-size:15px;color:var(--text-body);line-height:1.85;margin-bottom:18px">
                                                "{{ $t->review }}"</p>
                                            <img src="{{ url($t->reviewer_image) }}" alt="{{ $t->reviewer_name }}"
                                                style="width:48px;height:48px;border-radius:50%;object-fit:cover;margin:0 auto 8px;display:block;border:2px solid var(--border-strong)" />
                                            <strong
                                                style="font-family:'DM Sans',sans-serif;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:2px;color:var(--rose)">{{ $t->reviewer_name }}</strong>
                                            @if ($t->review_subtext)
                                                <p
                                                    style="font-size:10px;color:var(--text-muted);margin-top:3px;letter-spacing:0.5px">
                                                    {{ $t->review_subtext }}</p>
                                            @endif
                                        </div>
                                    </div>
                    ...
                    <div class="section gsap-fade">
                        ...
                        <div class="pay-links">
                            @foreach ($payment_details as $payment)
                                ...
                                @else
                                    <div class="gem-card" style="padding:16px;width:100%">
                                        @include('templates.partials.payment-link-image')
                                        <h4 style="font-size:14px;margin-top:8px">{{ $payment->label }}</h4>
                ...
                {{-- ── SOCIAL LINKS ── --}}
                @php
                    $socialTypes = ['facebook', 'twitter', 'linkedin', 'instagram', 'tiktok', 'pinterest', 'snapchat'];
                    $socialFeatures = collect($feature_details)->filter(fn($f) => in_array($f->type, $socialTypes));
                @endphp
                @if ($socialFeatures->isNotEmpty())
                    <div class="section gsap-fade">
                        <div class="section-header">
                            <h2 class="section-title">{{ __('Connect') }}</h2>
                            <div class="section-deco"><span class="section-deco__dash"></span><span
                                    class="section-deco__diamond"></span><span class="section-deco__dash"></span>
                            </div>
                        </div>
                            ...
                            @if ($feature->type == 'map')
                                <div class="map-wrapper">
                                    <iframe src="https://www.google.com/maps/embed?{!! $feature->content !!}"
                                        width="100%" height="220" style="border:0;display:block" allowfullscreen
                                        loading="lazy"></iframe>
                                    @if ($feature->label)
                        ...
                        <div class="wallet-section">
                            ...
                            @if ($google_wallet_details->wallet_link != null)
                                <a href="{{ $google_wallet_details->wallet_link }}" target="_blank"
                                    rel="noopener noreferrer" style="display:block;max-width:260px;margin:0 auto">
                ...
                @if ($plan_details['contact_form'] == 1 && $business_card_details->enquiry_email != null)
                    <div class="section gsap-fade" id="contact-section">
                        <div class="section-header">
                            <h2 class="section-title">{{ __($business_card_details->contact_form_title) }}</h2>
                            <div class="section-deco"><span class="section-deco__dash"></span><span
                                    class="section-deco__diamond"></span><span class="section-deco__dash"></span>
                            </div>
                        </div>
                        ...
                        @endif
                        <form class="gem-card" style="padding:24px" action="{{ config('app.url') }}/sent-enquiry"
                            method="POST">
                            @csrf
                            ...
                            <textarea name="message" placeholder="{{ __('How may we assist you today?') }}" class="form-control"
                                rows="4" required></textarea>
                            @include('templates.includes.recaptcha', ['recaptchaId' => 'recaptcha-one'])
                            <button type="submit" class="btn-primary btn-outline">{{ __('Send Inquiry') }}</button>
                        </form>
                    </div>
                @endif
                ...
                <div class="bottom-nav gsap-nav">
                    ...
                    <a href="{{ config('app.url') }}/download/{{ $business_card_details->card_id }}"
                        class="nav-item"><i class="fas fa-user-plus"></i></a>
    ...
    <div id="appointmentModal" class="std-modal hidden">
        <div class="std-modal-box" onclick="event.stopPropagation()">
            ...
            <form id="appointmentForm">
                ...
                <div class="hidden"><input type="text" id="price" class="form-control" disabled /></div>
                @include('templates.includes.recaptcha', ['recaptchaId' => 'recaptcha-two'])
                <div class="std-modal-footer">
                    <button type="button" class="btn-cancel" onclick="toggleModal()">{{ __('Close') }}</button>
    ...
    <div id="shareModal" class="std-modal hidden" onclick="shareToggleModal(false)">
        <div class="std-modal-box share-modal-box" onclick="event.stopPropagation()">
            ...
            <div class="share-icons">
                <a href="{{ $shareComponent['facebook'] }}" target="_blank"><i class="fab fa-facebook-f"></i></a>
                <a href="{{ $shareComponent['twitter'] }}" target="_blank"><i class="fab fa-twitter"></i></a>
                <a href="{{ $shareComponent['linkedin'] }}" target="_blank"><i class="fab fa-linkedin-in"></i></a>
                <a href="{{ $shareComponent['whatsapp'] }}" target="_blank"><i class="fab fa-whatsapp"></i></a>
    ...
    <div id="whatsappModal" class="std-modal hidden" onclick="toggleWhatsAppModal(false)">
        <div class="std-modal-box" onclick="event.stopPropagation()">
            <button class="close-btn" onclick="toggleWhatsAppModal(false)"><i class="fas fa-times"></i></button>
            ...
            <div class="whatsapp-divider"><span class="whatsapp-divider__line"></span><span
                    class="whatsapp-divider__diamond">◆</span><span class="whatsapp-divider__line"></span></div>
    ...
    <div id="scanModal" class="std-modal hidden" onclick="toggleScanModal(false)">
        <div class="std-modal-box" onclick="event.stopPropagation()">
            ...
            <p class="qr-helper"><i
                    class="fas fa-share-alt"></i>{{ __('Share this QR to let others save your contact instantly') }}
            </p>
        </div>
    </div>
    ...
    @if ($business_card_details->password != null && Session::get('password_protected') == false)
        <div class="pw-modal">
            <div class="pw-modal-box">
                <i class="far fa-gem anim-sparkle"
                    style="font-size:44px;color:var(--primary);margin-bottom:20px;filter:drop-shadow(0 0 10px var(--primary))"></i>
                <h2>{{ __('Private Access') }}</h2>
                <p>{{ __('Enter your vcard Password') }}</p>
                <form action="{{ config('app.url') }}/check-password/{{ $business_card_details->card_id }}"
                    method="post">
                    @csrf
                    <input type="password" name="password" class="pw-input" placeholder="••••" required autofocus />
                    @if (Session::has('message'))
                        <div class="alert-error" style="margin-bottom:12px">{{ Session::get('message') }}</div>
                    @endif
                    <button type="submit" class="btn-primary">{{ __('Unlock Collection') }}</button>
                </form>
            </div>
        </div>
    @endif
    ...
    {{-- ============================================================ PWA MODAL ============================================================ --}}
    @if ($plan_details != null && $plan_details['pwa'] == 1 && $business_card_details->is_enable_pwa == 1)
        @include('vendor.laravelpwa.jewellery-shop-vcard')
    @endif
    ...
    @if (
        ...
        @include('templates.includes.vcard.jewellery-shop.newsletter_modal')
    @endif
    ...
    @if (
        ...
        @include('templates.includes.vcard.jewellery-shop.information_popup_modal')
    @endif
    ...
    <script>
        ...
        document.getElementById('appointmentForm')?.addEventListener('submit', function(e) {
            ...
            fetch("{{ config('app.url') }}/book-appointment", {
                ...
                btn.disabled = false;
                btn.innerHTML = `{{ __('Book Appointment') }}`;
            }).catch(() => {
                toggleModal();
                btn.disabled = false;
                btn.innerHTML = `{{ __('Book Appointment') }}`;
        ...
        function submitServiceBooking() {
            ...
            fetch("{{ config('app.url') }}/book-service", {
                ...
                if (data.success) {
                    ...
                    sucEl.innerHTML = data.message || 'Booked successfully!';
                } else {
                    errEl.classList.remove('hidden');
                    errEl.innerHTML = data.message || 'Something went wrong';
                }
