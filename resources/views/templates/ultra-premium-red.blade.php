                        ...
                        @if (isset($plan_details['service_booking']) && $plan_details['service_booking'] == 1)
                            @if (isset($service_booking_details) && $service_booking_details->service_booking == 1)
                                @include('templates.includes.service-booking', [
                                    ...
                                    'head_style' => 'bg-red-600 w-full mt-4 mb-4 flex justify-center align-middle py-2',
                                    'input_style' => 'w-full p-4 text-xs bg-gray-100 outline-none rounded',
                                    'btn_style' => 'mb-2 w-full py-4 bg-red-600 text-sm rounded-full font-bold text-gray-50 transition duration-200',
                                ])
                            @endif
                        @endif
                        {{-- End Service Booking --}}
                        ...
                        @if ($plan_details['hide_branding'] == 1)
                            <div class="pb-1">
                                <div
                                    ...
                                    <div class="mt-2">
                                        {{ __('Copyright') }} &copy;
                                        @php $__cr = $business_card_details->copyright ?? ''; $__cr_is_domain = !empty($__cr) && (str_starts_with($__cr, 'http') || str_contains($__cr, '.')); $__cr_url = $__cr_is_domain ? (str_starts_with($__cr, 'http') ? $__cr : 'https://'.$__cr) : (!empty($__cr) ? null : env('APP_URL')); $__cr_text = !empty($__cr) ? $__cr : parse_url(config('app.url'), PHP_URL_HOST); @endphp @if($__cr_url)<a class="text-red-700" href="{{ $__cr_url }}" target="_blank" rel="noopener noreferrer">{{ $__cr_text }}</a>@else<span class="text-red-700">{{ $__cr_text }}</span>@endif
