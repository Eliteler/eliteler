...
<h1 class=" text-3xl font-medium text-[#121212] custom-head" style="margin-bottom: 0px; padding-bottom: 0px;">{{ $business_card_details->title2 }}</h1>
@endif
@if(!empty($business_card_details->subtitle2))
<p class="text-[#327C91] font-medium mt-2 text-md" style="margin-bottom: 5px; padding-bottom: 0px;">{{ $business_card_details->subtitle2 }}</p>
@endif
<h1 class=" text-3xl font-medium text-[#121212] custom-head">{{ $business_card_details->title }}</h1>
                                    {{-- Job Title --}}
                                    <p class="text-[#327C91] font-medium mt-2 text-md">{{ $card_details->sub_title }}</p>
                                    {{-- About --}}
                                    ...
                                    <div class="pb-1">
                                        <div
                                            ...
                                            <div class="mt-2 text-gray-500">
                                                {{ __('Copyright') }} &copy;
                                                @php $__cr = $business_card_details->copyright ?? ''; $__cr_is_domain = !empty($__cr) && (str_starts_with($__cr, 'http') || str_contains($__cr, '.')); $__cr_url = $__cr_is_domain ? (str_starts_with($__cr, 'http') ? $__cr : 'https://'.$__cr) : (!empty($__cr) ? null : env('APP_URL')); $__cr_text = !empty($__cr) ? $__cr : parse_url(config('app.url'), PHP_URL_HOST); @endphp @if($__cr_url)<a class="text-[#327C91]" href="{{ $__cr_url }}" target="_blank" rel="noopener noreferrer">{{ $__cr_text }}</a>@else<span class="text-[#327C91]">{{ $__cr_text }}</span>@endif<span id="year"></span>{{ __('. All Rights Reserved.') }}
                                            </div>
                                        </div>
                                    </div>
                                @else
                                    <div class="pb-1">
                                        <div
                                            ...
                                            <div class="mt-2 text-gray-500">
                                                {{ __('Made with') }}
                                                @php $__cr = $business_card_details->copyright ?? ''; $__cr_is_domain = !empty($__cr) && (str_starts_with($__cr, 'http') || str_contains($__cr, '.')); $__cr_url = $__cr_is_domain ? (str_starts_with($__cr, 'http') ? $__cr : 'https://'.$__cr) : (!empty($__cr) ? null : env('APP_URL')); $__cr_text = !empty($__cr) ? $__cr : parse_url(config('app.url'), PHP_URL_HOST); @endphp @if($__cr_url)<a class="text-[#327C91]" href="{{ $__cr_url }}" target="_blank" rel="noopener noreferrer">{{ $__cr_text }}</a>@else<span class="text-[#327C91]">{{ $__cr_text }}</span>@endif
