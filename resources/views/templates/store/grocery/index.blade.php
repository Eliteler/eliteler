...
<head>
    ...
    <meta name="theme-color" content="grocery-green" />
    ...
    <!-- Tile for Win8 -->
    <meta name="msapplication-TileColor" content="grocery-green">
    ...
    <div class="page page-center preloader-wrapper">
        <div class="container container-slim py-4">
            <div class="text-center">
                <div class="spinner-border text-grocery-green" role="status"></div>
            </div>
        </div>
    </div>

    {{-- Page --}}
    <div id="wrapper" class="page m-0 min-h-screen" style="background-color: #C8E6C9;">
        <div class="page-wrapper m-0 p-0">
            <div class="page-body m-0 p-0">
                <div class="container-xl card rounded-0 p-0 overflow-hidden" style="min-height: 100vh;">
                    <!-- Navbar -->
                    @include('templates.store.grocery.includes.header', [
                        'bg_color' => 'grocery-green',
                        'badge_color' => 'danger',
                    ])
                    ...
                    <div class="row mt-7 px-3 px-lg-4">
                        {{-- Greeting message --}}
                        <div class="col-md-12">
                            <h3 class="alert alert-important alert-grocery-green p-3">
...
{{ $business_card_details->sub_title }}
                            ...
                            <swiper-container space-between="20" class="mySwiper" autoplay-delay="2500"
                                ...
                                @if (is_array(json_decode($business_card_details->cover)) == true)
                                    @foreach (json_decode($business_card_details->cover) as $cover)
                                        <swiper-slide>
                                            <img class="rounded-grocery w-100 h-100 object-fit-cover"
                        ...
                        <div class="row {{ request('category') ? 'ps-2 ps-lg-3' : '' }}">
                            <!-- Categories -->
                            <div
                                class="{{ request('category') ? 'bg-opacity-green rounded-grocery col-md-3 my-3 px-3' : 'col-md-12 my-3' }}">
                                <div class="position-relative {{ request('category') ? 'd-none' : '' }}">
                                    <img class="position-absolute rotate-loop"
                                        src="{{ asset('img/templates/grocery/1.png') }}"
                                        style="
                                            width: 130px;
                                            height: 130px;
                                            left: -55px;
                                ...
                                <form class="row pt-3 {{ request('category') ? '' : 'd-none' }}" method="GET"
                                    ...
                                    <div class="col-md-4 mt-2 mt-md-0">
                                        <button type="submit"
                                            class="btn btn-grocery-green w-100">{{ __('Filter') }}</button>
                                    </div>
                                </form>
                                ...
                                <h2
                                    ...
                                    {{ __('Categories') }}
                                    <div
                                        class="position-absolute start-50 translate-middle-x bg-grocery-green bottom-bar2">
                                    </div>
                                </h2>
                                ...
                                <h2
                                    ...
                                    {{ __('Categories') }}
                                    <div class="position-absolute start-0 bg-grocery-green bottom-bar2"></div>
                                </h2>
                                ...
                                <swiper-container space-between="10"
                                    ...
                                    @foreach ($categories as $category)
                                        <swiper-slide class="cursor-pointer h-100">
                                            <a
                                                href="{{ url($business_card_details->card_url) . '?category=' . urlencode(strtolower($category->category_name)) }}">
                                                <div class="card rounded-pill h-100 border-green">
                                                    <div
                                                        class="card-body d-flex flex-row align-items-center gap-3 p-2">
                                                        <!-- Thumbnail -->
                                                        <div class="rounded-circle bg-opacity-green border border-green flex-shrink-0"
                                                            style="width: 68px; height: 68px;">
                                                            <img src="{{ url($category->thumbnail) }}"
                                                                class="object-fit-cover h-100 w-100 rounded-circle"
                                                                alt="{{ $category->category_name }}" />
                                                        </div>
                                                        <!-- Category Name -->
                                                        <div class="flex-grow-1 overflow-hidden">
                                                            <h3 class="fs-2 text-truncate single-product m-0">
                            ...
                            <!-- Products -->
                            <div class="mt-2 {{ request('category') ? 'col-md-9 ps-lg-4' : 'col-md-12' }}">
                                <div class="position-relative {{ request('category') ? 'd-none' : '' }}">
                                    <img class="position-absolute" src="{{ asset('img/templates/grocery/4.png') }}"
                                        style="
                                            width: 115px;
                                            height: 115px;
                                            right: -30px;
                                ...
                                <div class="position-relative {{ request('category') ? '' : 'd-none' }}">
                                    <img class="position-absolute blur-img"
                                        src="{{ asset('img/templates/grocery/5.png') }}" />
                                </div>
                                <div class="row align-items-center {{ request('category') ? 'mt-2' : 'd-none' }}">
                                    ...
                                    <form class="row col-md-8" method="GET" action="{{ url()->current() }}">
                                        ...
                                        <div class="col-3">
                                            <button type="submit"
                                                class="btn btn-grocery-green w-100">{{ __('Search') }}</button>
                                        </div>
                                    </form>
                                    ...
                                    <div class="row">
                                        ...
                                        @foreach ($products as $product)
                                            <div
                                                ...
                                                <div class="card border-green d-flex flex-column p-3">
                                                                ...
                                                                <div class="carousel-inner">
                                                                    @foreach ($productImages as $productImage)
                                                                        <div
                                                                            ...
                                                                            <img class="d-block w-100 rounded-grocery"
                                                        ...
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            ...
                                                            @if ($product->product_status == 'instock')
                                                                <a onclick="addToCart('{{ $product->product_id }}')"
                                                                    ...
                                                                    <i
                                                                        class="ti ti-shopping-bag-plus bg-grocery-green text-white p-custom fs-1 rounded-grocery"></i>
                                                                </a>
                                                            @endif
                                                        </div>
                                                    </div>
                                        ...
                                        <div class="col-md-12 my-3">
                                            @if (request()->has('category'))
                                                {{ $products->appends(request()->except('page'))->links('vendor.pagination.store', ['color' => 'grocery-green']) }}
                                            @else
                                                {{ $products->links('vendor.pagination.store', ['color' => 'grocery-green']) }}
                                            @endif
                                        </div>
                                    </div>
                                @else
                                        ...
                                        <ul class="list-inline list-inline-dots mb-0">
                                            <li class="list-inline-item">
                                                <a href="{{ $shareComponent['facebook'] }}" target="_blank"
                                                    class="link-light">
                                                    <i class="ti ti-brand-facebook-filled text-grocery-green"></i>
                                                </a>
                                            </li>
                                            <li class="list-inline-item">
                                                <a href="{{ $shareComponent['twitter'] }}" target="_blank" class="link-light">
                                                    <i class="ti ti-brand-twitter-filled text-grocery-green"></i>
                                                </a>
                                            </li>
                                            <li class="list-inline-item">
                                                <a href="{{ $shareComponent['linkedin'] }}" target="_blank"
                                                    class="link-light">
                                                    <i class="ti ti-brand-linkedin text-grocery-green"></i>
                                                </a>
                                            </li>
                                            <li class="list-inline-item">
                                                <a href="{{ $shareComponent['telegram'] }}" target="_blank"
                                                    class="link-light">
                                                    <i class="ti ti-brand-telegram text-grocery-green"></i>
                                                </a>
                                            </li>
                                            <li class="list-inline-item">
                                                <a href="{{ $shareComponent['whatsapp'] }}" target="_blank"
                                                    class="link-light">
                                                    <i class="ti ti-brand-whatsapp text-grocery-green"></i>
                                                </a>
                                            </li>
                                        </ul>
                                    </div>
        ...
        <!-- Bottom Navbar -->
        @include('templates.includes.bottom-bar', ['color' => 'grocery-green', 'bg' => 'light'])
        <!-- End Bottom Navbar -->
    </div>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasEnd" aria-labelledby="offcanvasEndLabel">
        ...
        <div class="offcanvas-body">
            ...
            <div id="empty-cart" class="p-3">
                ...
                <!-- Start Shopping -->
                <a class="btn btn-grocery-green d-flex btn-effect" data-bs-dismiss="offcanvas"
        ...
        <div class="offcanvas-footer border-top">
            ...
            <!-- Place Order -->
            <a class="btn btn-grocery-green fs-2 btn-effect" id="place-order"
        ...
        <div class="modal-dialog modal-md modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <!-- Confirm -->
                    <button type="button" class="btn btn-grocery-green"
                        ...
                        <div class="modal-body">
                            <form action="{{ config('app.url') }}/subscribe/store/newsletter" method="post">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-4">
                                        <button type="submit"
                                            class="btn btn-grocery-green w-100">{{ __('Subscribe') }}</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                            ...
                            <div class="row d-flex justify-content-center align-items-center">
                                ...
                                <div class="d-flex justify-content-center align-items-center">
                                    <a href="{{ $button_url }}" target="_blank"
                                        class="btn btn-grocery-green w-100">{{ __($button_text) }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function updateList() {
            ...
            cart.forEach((item, index) => {
                ...
                cart_items += `<div class="card border-green p-3">`;
                cart_items += `<div class="d-flex align-items-center">`;

                // LEFT: Image
                cart_items += `
                <div class="me-3 flex-shrink-0" style="width: 100px; height: 100px;">
                    <img src="${item.product_image}" class="img-fluid rounded-grocery object-fit-cover h-100 w-100" alt="${item.product_name}" />
                </div>`;
            ...
            if (grandTotal > 0) {
                ...
                $("#cart-pricing").html(
                    `<h3 class="font-bold fs-2 d-flex justify-content-between"><span>{{ __('Grand total') }}</span><span class="text-grocery-green">${jsFormatCurrency(grandTotal, setDecimalsPlaces, formatType)}</span></h3>`
                );
            } else {
                $("#cart-pricing").html(``);
            }
        ...
        function updateBadge() {
            ...
            if (badgeCount > 0) {
                ...
                $("#place-order").show().attr("class", "btn btn-grocery-green d-flex");
            } else {
                $("#place-order").hide().attr("class", "btn btn-grocery-green d-none");
                $("#badge").hide();
                $("#empty-cart").show();
            }
        }
